    ...
    <form action="{{ url('/admin/categories/' .$categories->id) }}" method="post">
        ...
        <select name="game_id" id="game_id" class="form-control @error('game_id') is-invalid @enderror">
          ...
          @foreach ($games as $item)
            <option value="{{ $item->id }}" {{ old('game_id', $categories->game_id) == $item->id ? 'selected' : null }}>{{ $item->name }}</option>
          @endforeach
        </select>
        @error('game_id')
        ...
        <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $categories->name) }}">
        @error('name')
          <div class="invalid-feedback">{{ $message }}</div>
        @enderror
        <br>
